...
#[derive(Accounts)]
#[instruction(params: QuoteDepositParams)]
...
impl<'info> QuoteDeposit<'info> {
    pub fn apply(ctx: &Context<QuoteDeposit>, params: &QuoteDepositParams) -> Result<MessagingFee> {
        let message = msg_codec::encode_deposit(ctx.accounts.store.key(), params.receiver,&params.message)?;
